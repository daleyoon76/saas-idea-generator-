...
- [ ] **경쟁 분석**
  - [ ] Manus AI 기능/가격 분석 → 보완점 도출
  - [ ] 기타 경쟁 서비스 리스트업
- [ ] **에이전트 파이프라인 안정화**
  - [x] Devil's Advocate RISK_SUMMARY → Section 1 삽입 ✅
  - [x] 초안을 풀버전 에이전트 컨텍스트로 제공 ✅
  - [ ] 섹션 추출 강화 (누락/순서 뒤바뀜 감지 개선)
  - [ ] 에이전트 실패 복구 (개별 재시도 / 부분 결과 보존)
  - [ ] Claude Opus 풀버전 생성 테스트 (타임아웃·페이로드 검증)
- [ ] **검색 품질 강화**
  - [ ] Tavily 쿼리 최적화 (하드코딩 5개 → 동적 생성)
  - [ ] 검색 결과 캐싱 (동일 키워드 재검색 방지)
- [ ] **출력물 품질 관리**
  - [x] 서버 출력 검증 (LLM 잘림 감지 + 섹션 누락 경고 + blockquote 경고) ✅
  - [ ] 서버 출력 검증 수동 테스트 (test-validation 4개 시나리오, Gemini Flash 잘림 확인, Claude Sonnet 정상 확인)
  - [ ] Mermaid 렌더 실패 시 안내 메시지 표시
