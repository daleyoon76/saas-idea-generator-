...
- [ ] **Devil's Advocate 강화** — `docs/agents_jd.md` DA 지시문 수정 (파이프라인 구조 변경 없음)
  - [ ] 한국형 규제 리스크 자동 점검 — 인허가, 개인정보보호법, 전자상거래법 등 체크리스트 추가
  - [ ] 스트레스 테스트 시나리오 — '경쟁사 가격 20% 인하', '핵심 인력 이탈' 등 자동 시뮬레이션
  - [ ] A/B 비교: 기존 DA vs 수정 DA 기획서 품질 확인 (6K 토큰 한도 내 기존 분석 깊이 유지되는지)
...
- [ ] **멀티에이전트 팀 협업 구조 심화 + Devil's Advocate 한국형 튜닝** (기존 보유 → 고도화)
  ...
  - [ ] Devil's Advocate 스트레스 테스트 — '경쟁사 가격 20% 인하 시' 시나리오 자동 실행
  - [ ] Devil's Advocate 한국형 튜닝 — 한국 규제 환경(인허가, 개인정보보호법) 리스크 자동 점검
...
- [ ] **출처 강제 / 할루시네이션 억제 시스템 고도화** (기존 보유 → 고도화)
  ...
  - [ ] 출처 URL 자동 검증(dead link 체크) + 출처 신뢰도 등급 시각화
