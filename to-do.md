...
- [ ] **에이전트 파이프라인 안정화**
  - Devil's Advocate(Agent 5) 검토 결과를 본문에 반영하는 로직 재설계
  - 에이전트 간 토큰 예산 최적화 (현재 개인용 3x → 프로덕션 기준 조정)
  - 초안을 Agent 1 컨텍스트로 제공 → 풀버전 품질 향상
- [ ] **검색 품질 강화**
  - Tavily 쿼리 최적화 (하드코딩 5개 → 동적 생성)
  - 검색 결과 캐싱 (동일 키워드 재검색 방지)
- [ ] **출력물 품질 관리**
  - LLM 출력 구조 검증 (섹션 누락/순서 뒤바뀜 감지)
  - .docx 서식 개선 (표 스타일, 페이지 레이아웃)
- [ ] **테스트 프레임워크 도입**
  - Vitest + React Testing Library (단위/통합)
  - Playwright (E2E: 아이디어 생성→기획서→저장 전체 흐름)
  - 핵심 경로 60%+ 커버리지 목표
